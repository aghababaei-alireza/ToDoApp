...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        # Create fake user
        user = User.objects.create_user(email=self.fake.email(), password="fake@123456", is_verified=True)
        ...
        for _ in range(5):
            Task.objects.create(
                ...
                due_date=datetime.now(tz=timezone.utc) + self.fake.time_delta(end_datetime="+30d"),
